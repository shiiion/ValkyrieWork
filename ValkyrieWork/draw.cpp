...
namespace renderer
{
	...
	void CDraw::Line(float x1, float y1, float x2, float y2, float width, bool antialias, DWORD color) const
	{
		ID3DXLine* m_Line;
	...
	void CDraw::Circle(float x, float y, float radius, int rotate, int type, bool smoothing, int resolution, DWORD color)
	{
		...
		const auto angle = rotate * D3DX_PI / 180;
		float pi;
		...
		VOID* pVertices;
		g_pVB->Lock(0, (resolution + 2) * sizeof(vertex), static_cast<void**>(&pVertices), 0);
	...
	void CDraw::BoxFilled(float x, float y, float w, float h, DWORD color)
	{
		...
		pDevice->CreateVertexBuffer(
			4 * sizeof(vertex), D3DUSAGE_WRITEONLY, D3DFVF_XYZRHW | D3DFVF_DIFFUSE, D3DPOOL_DEFAULT, &g_pVB, nullptr);
		pDevice->CreateIndexBuffer(2 * sizeof(short), D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, D3DPOOL_DEFAULT, &g_pIB, nullptr);

		VOID* pVertices;
		g_pVB->Lock(0, sizeof(V), static_cast<void**>(&pVertices), 0);
		memcpy(pVertices, V, sizeof(V));
		g_pVB->Unlock();

		VOID* pIndex;
		g_pIB->Lock(0, sizeof(indexes), static_cast<void**>(&pIndex), 0);
	...
	void CDraw::DrawString(const wchar_t* text, float x, float y, int orientation, bool bordered, DWORD color, DWORD bcolor)
	{
		RECT rect;
		...
		if (bordered)
		{
			SetRect(&rect, xi - 1, yi, xi - 1, yi);
			pFont[0]->DrawTextW(nullptr, text, -1, &rect, format, bcolor);
			SetRect(&rect, xi + 1, yi, xi + 1, yi);
			pFont[0]->DrawTextW(nullptr, text, -1, &rect, format, bcolor);
			SetRect(&rect, xi, yi - 1, xi, yi - 1);
			pFont[0]->DrawTextW(nullptr, text, -1, &rect, format, bcolor);
			SetRect(&rect, xi, yi + 1, xi, yi + 1);
			pFont[0]->DrawTextW(nullptr, text, -1, &rect, format, bcolor);
		}
		SetRect(&rect, xi, yi, xi, yi);
		pFont[0]->DrawTextW(nullptr, text, -1, &rect, format, color);
	}

	void CDraw::DrawString(const char* text, float x, float y, int orientation, bool bordered, DWORD color, DWORD bcolor)
	{
		RECT rect;
		...
		{
			SetRect(&rect, xi - 1, yi, xi - 1, yi);
			pFont[0]->DrawTextA(nullptr, text, -1, &rect, format, bcolor);
			SetRect(&rect, xi + 1, yi, xi + 1, yi);
			pFont[0]->DrawTextA(nullptr, text, -1, &rect, format, bcolor);
			SetRect(&rect, xi, yi - 1, xi, yi - 1);
			pFont[0]->DrawTextA(nullptr, text, -1, &rect, format, bcolor);
		...
		SetRect(&rect, xi, yi, xi, yi);
		pFont[0]->DrawTextA(nullptr, text, -1, &rect, format, color);
	}

	void CDraw::Message(const char* text, float x, float y, int font, int orientation) {}

	void CDraw::Sprite(LPDIRECT3DTEXTURE9 tex, float x, float y, float resolution, float scale, float rotation) const
	{
		const auto screen_pos_x = x;
		const auto screen_pos_y = y;

		// Texture being used is 64x64:
		auto spriteCentre = D3DXVECTOR2(resolution / 2, resolution / 2);

		// Screen position of the sprite
		auto trans = D3DXVECTOR2(screen_pos_x, screen_pos_y);
		...
		sSprite->Draw(tex, nullptr, nullptr, nullptr, 0xFFFFFFFF);
		sSprite->End();
	}

	void CDraw::AddFont(const char* FontName, float size, bool bold, bool italic)
	{
		D3DXCreateFont(
			pDevice,
			size,
